...
const McqFile = ({
  ...
  // navigate so that the user may navigate to the update mcq page
  ...
  const handleButtonClick = (e) => {
    ...
    // console.log("this is from mcqFile.jsx", whichButton);
    if (whichButton === "delete") {
      const res = deleteFile(fileId);
      if (res === false) {
        alert("Server Error :: mcq cannot be deleted");
      }
      ...
      navigate(`/mcq/update/${filename}`);
    }
  };

  return (
    <>
      <Link to={`/mcq/${filename}`}>
        <Card
          maxW={350}
          display="flex"
